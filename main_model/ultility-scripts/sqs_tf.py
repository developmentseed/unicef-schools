...
Populate an SQS queue from a list of geojson boundaries, and sent the queue for large
scale machine learning inference

@author:developmentseed

use:
python3 sqs_tf.py --inds_txt_files ind1.txt ind2.txt ind3.txt
"""
...
def send_tile_msg_batch(queue, msg_template, batch_inds):
    ...
    entries = []
    for id_num, (x, y, z) in enumerate(batch_inds):
        entries.append({'MessageBody':msg_template.format(x=x, y=y, z=z),
                        'Id':str(id_num)})
...
def send_tile_msg(queue, msg_template, x, y, z):
    """Send a single message to SQS queue."""
    msg_body = msg_template.format(x=x, y=y, z=z)
    try:
...
def main(files):
    # files = list(files)
    msg_template='{{"x":{x},"y":{y},"z":{z}}}'
    sqs = boto3.resource('sqs', region_name='us-east-1')
    queue = sqs.Queue('https://sqs.us-east-1.amazonaws.com/xxxx/MLInferenceUNICEFTileQueue')
